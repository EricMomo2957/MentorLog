...
const StudentLayout = ({ children }: { children: React.ReactNode }) => {
    ...
    const [isCollapsed, setIsCollapsed] = useState<boolean>(() => {
        return localStorage.getItem('mentorlog_student_sidenav_collapsed') === 'true';
    });

    const toggleSidebar = () => {
        setIsCollapsed(prev => {
            const next = !prev;
            localStorage.setItem('mentorlog_student_sidenav_collapsed', String(next));
            return next;
        });
    };
    ...
    let currentPageLabel = 'Student Portal';
    navGroups.forEach(g => {
            ...
            <aside className={`bg-[#0b1329] text-slate-300 flex flex-col sticky top-0 h-screen z-50 shrink-0 border-r border-slate-800 shadow-xl transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-64'}`}>
                    ...
                    {!isCollapsed ? (
                        <div className="flex items-center gap-3 overflow-hidden">
                            ...
                            <div className="min-w-0">
                                ...
                                <span className="text-[10px] text-blue-400 font-medium truncate block">Student Portal</span>
                            </div>
                        </div>
                    ) : (
                        <div className="w-9 h-9 rounded-xl bg-blue-600 flex items-center justify-center text-white shadow-md shadow-blue-600/30 shrink-0" title="MentorLog Student Portal">
                            <img src={logoPhoto} alt="Logo" className="w-6 h-6 object-contain rounded" />
                        </div>
                    )}
                ...
                {!isCollapsed ? (
                    ...
                    <div className="mx-2 my-4 flex justify-center">
                        <Link to="/student-profile" title={`${userName} (${userCourse || 'Student Intern'})`} className="hover:opacity-80 transition-opacity">
                            {picUrl ? (
                                <img 
                                    src={picUrl} 
                                    alt={userName} 
                ...
                <div className="p-3 border-t border-slate-800/80 space-y-1 shrink-0">
                    <button 
                        ...
                        className={`w-full flex items-center gap-3 ${isCollapsed ? 'justify-center p-2.5' : 'px-3.5 py-2.5'} rounded-xl text-xs font-medium text-slate-400 hover:text-red-400 hover:bg-red-500/10 transition-all group cursor-pointer`}
                    >
            ...
            {isLogoutModalOpen && (
                <div className="fixed inset-0 z-100 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white w-full max-w-sm rounded-2xl border border-slate-200 shadow-2xl p-6 text-center animate-in zoom-in-95 duration-200">
                        ...
                        <h3 className="text-xl font-bold text-slate-800 mb-1">Sign Out?</h3>
                        <p className="text-slate-500 text-xs mb-6">
                            Make sure your OJT tasks are saved before signing out of <span className="font-semibold text-slate-700">MentorLog</span>.
                        </p>
                        ...
                        <div className="flex gap-3">
                            ...
                            <button 
                                ...
                                className="w-1/2 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-xl font-semibold text-xs transition-all shadow-md shadow-red-600/20"
                            >
                                Sign Out
                            </button>
                        </div>
